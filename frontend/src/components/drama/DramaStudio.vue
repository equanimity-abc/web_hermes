...
function voiceOverlayText() {
  // 字幕 = 剧情说明（画面浮层）；对白走配音，不叠在画面上
...
function voiceOverlayIsMonologue() {
  const raw = String(props.draft?.字幕 || '').trim()
  if (!raw) return false
  if (/(?:内心独白|心声|OS)/i.test(raw)) return true
  const text = voiceOverlayText()
  if (!text || /[·・]/.test(text) || /^第[一二三四五六七八九十百千\d]+[集章回]/.test(text)) return false
  if (/^[他她您]/.test(text)) return false
  return /我|自己|这一次|这一世|不会再/.test(text)
}
          ...
          <div v-if="selected" class="drama-scene-detail">
            <div class="drama-scene-detail-head">
              <div class="drama-scene-detail-title">
                <h3>Shot {{ selected.n }}</h3>
                <p class="drama-shot-subhint">时长取自剧本分镜；生成前请先在「画面」锁定关键帧。L0 定场镜仅静图运镜。</p>
              </div>
              <div class="drama-scene-detail-actions">
        ...
        <div class="drama-panel-body drama-scene-layout">
          ...
          <div v-if="selected" class="drama-scene-detail">
            <div class="drama-scene-detail-head">
              <div class="drama-scene-detail-title">
                <h3>Shot {{ selected.n }}</h3>
                <p class="drama-shot-subhint">对白用于配音；字幕为剧情说明，浮在画面上。可编辑后保存。</p>
              </div>
              <div class="drama-scene-detail-actions">
                <button type="button" class="btn-ghost btn-sm" :disabled="saving || !dirty" @click="emit('save')">
                  {{ saving ? '保存中…' : dirty ? '保存' : '已保存' }}
            ...
            <div class="drama-scene-main">
              <div class="drama-video-preview drama-voice-preview">
                <video
                ...
                <div
                  v-if="voiceOverlayText()"
                  class="drama-voice-sub-layer"
                  :class="{ 'is-monologue': voiceOverlayIsMonologue() }"
                  aria-hidden="true"
                >
                  <p class="drama-voice-sub-text" :class="{ 'is-monologue': voiceOverlayIsMonologue() }">
                    {{ voiceOverlayText() }}
                  </p>
                </div>
              </div>
              ...
              />
              <p v-else class="drama-empty-hint" style="margin-top: 10px">尚未生成配音</p>
            </div>
          </div>

          <div v-else class="drama-cast-empty">
            <p>从左侧选择一镜，查看对白并生成配音与口型。</p>
          </div>
        </div>
      </section>
