...
Invariants:
  ...
  - Insert a user/assistant bridge pair so role alternation stays valid.
"""
...
def _snap_recent_start(messages: list[dict[str, Any]], start: int) -> int:
    ...
    while start < len(messages) and messages[start].get("role") == "tool":
        ...
        if start <= 1:
            break
    # If we landed on assistant with tool_calls, keep the whole group in recent
    # (start already points at that assistant).
    return max(1, start)
...
async def _llm_summary(client: LLMClient, transcript: str) -> str:
    ...
    msg = await client.chat_completion(prompt, tools=None, temperature=0.2, max_tokens=600)
...
async def maybe_compress(
    ...
    *,
    ...
    # Keep system (index 0) + last keep_recent messages
    ...
    bridge = [
        ...
        {
            "role": "assistant",
            "content": "好的，我已了解此前对话的要点，我们继续。",
        },
    ]

    # Avoid double user if recent starts with user — still OK for OpenAI APIs,
    # but prefer not stacking two users: merge summary into first recent user.
    if recent and recent[0].get("role") == "user":
        first = dict(recent[0])
        first["content"] = (
            f"[此前对话摘要]\n{summary}\n\n---\n" + str(first.get("content") or "")
        )
        new_messages = head + [first] + recent[1:]
    else:
        new_messages = head + bridge + recent
