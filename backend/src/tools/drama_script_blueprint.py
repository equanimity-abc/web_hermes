...
def build_episode_script_system(
    *,
    ...
    from tools.drama_ark_prompts import SHOT_FRAME_WRITING_RULE

    return (
        "你是专业竖屏漫剧编剧与视觉统筹。只输出【一支】完整短片剧本 Markdown，"
        "不要输出任何多余说明，禁止写多集。\n\n"
        "目标：先建立完整连贯的结构化设定，再写可拍分镜；"
        "后续定妆、地点主底板、道具设定图、配音、成片配乐都必须能直接引用这些字段，"
        "因此名称与细节必须前后一致、可拍摄、可复现。\n\n"
        f"{title_line}\n"
        f"- 时长: {ep_sec}s\n"
        "- 钩子: 前3秒抓住观众的一句话\n"
        "- 悬念: 结尾反转或未解悬念\n"
        ...
        "- 性格: 2–4 词或短句\n"
        "- 音色倾向: 如女声温柔 / 男声低沉 / 少年清亮（供 Seed Audio 选型；可写方言如四川话/粤语）\n"
        "- 口头禅: 可选\n"
        ...
        "### 场景名\n"
        "- 描述: 空间结构（进深/左右分区）、建筑轮廓、地面材质、主陈设、时代气质；"
        "写清「站在哪里能看见什么」\n"
        "- 光影色调: 主光方向（如左上方冷月光）、冷暖对比、主色与辅色（尽量可写成色值或明确色名）\n"
        "- 标志物: 1–3 个固定视觉锚（飞檐/玉阶/桂树/灯笼等），跨镜必须反复出现\n\n"
        "## 道具设定\n"
        "只写推动剧情的关键道具（通常 1–4 个）；名称稳定，分镜引用同名。\n"
        "### 道具名\n"
        "- 描述: 一眼可辨的外形与功能\n"
        "- 材质外形: 材质、尺寸感、纹样、磨损/光泽等可画细节\n"
        "- 剧情作用: 与冲突/身份/反转的关系\n\n"
        "## 分镜\n"
        f"### Shot 1 (0-3s)\n"
        "- 画面: 景别+构图+人物动作+镜头运动感；环境只点到与地点标志物相关的可见细节，"
        "禁止每镜发明新背景；禁止写切黑/转场指令；"
        f"{SHOT_FRAME_WRITING_RULE}\n"
        "- 地点: 必须精确引用「场景设定」中的场景名（逐字一致）\n"
        "- 道具: 本镜可见的关键道具（顿号分隔；名称引用「道具设定」，可空）\n"
        "- 字幕: 角色台词；格式「角色名：台词」；口语化、可配音；无对白可空（勿写「无」）\n"
        "- 旁白: 画外说明（左上竖排）；可空则省略该行或留空，禁止填写「无」「没有」等占位\n"
        ...
        "关键道具写入道具字段且与道具设定同名；"
        "台词与旁白服务情绪与信息推进；"
        "对白镜字幕格式固定为「角色短名：台词」，便于 Seedance 自带声或手动配音对齐口型；"
