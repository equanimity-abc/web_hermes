...
def build_assemble_specs(
    ...
    *,
    ...
    for shot in ordered_shots(doc):
        ...
        if not path.is_file():
            source = float(shot.get("duration") or 5)
            tl = normalize_shot_timeline(shot)
            ...
            continue
        source = float(probe_duration(path) or shot.get("duration") or 5)
        tl = normalize_shot_timeline(shot)
        play = play_duration(shot, source)
        specs.append(
            {
