...
**漫剧草稿成片（P7–P8 + D0–D6）已齐：** 插件 `tiktok_drama` 能立项 / 写 bible·角色卡·大纲·分集；`shots.json` 单镜重渲；工作台可改对白/运镜；锁住画面后改台词只换声和字幕；剧本页改结局时已锁整镜不动；同一角色连续镜头共用外形 prompt 与音色；每镜候选墙点选换图不重配音；时间线页改镜序/切点/转场/音量后导出整集，不覆盖源 clip。

**下一刀：D7 任务条。** D6 已能拖拽镜序、裁切时长、选转场、调音量并立刻导出新 mp4。完整计划见下文 **漫剧智能体制作流水线（D0–D8）**。
...
| **D7–D8** | 队列 / I2V | 见专章 | 导演式干预到镜头层 | 见下文 D0–D8 | 计划中 |
...
P0–P8 + D0–D6 已齐。业务继续放插件与 `/api/drama/*`，**不要改 `agent/loop.py`**。

下一刀 **D7**：后台渲染、可取消、失败重试、工作台进度条。验收：渲 8 镜时聊天仍可用。
...
检查器目标：**保存 / 仅重做这一层 / 解锁 / 回滚版本**。D1 已有保存 + 重渲本镜；D2 已有锁与分层重做；D3 已有锁定整镜与剧本影响提示；D4 已有角色勾选与音色绑定；D5 已有候选墙点选换图（锁 scene，不重配音）；D6 已有时间线镜序/切点/转场/音量与导出不毁源 clip。回滚版本仍待做。
