...
**漫剧草稿成片（P7–P8 + D0–D3）已齐：** 插件 `tiktok_drama` 能立项 / 写 bible·大纲·分集；`shots.json` 单镜重渲；工作台可改对白/运镜；锁住画面后改台词只换声和字幕；剧本页改结局时已锁整镜不动。

**下一刀：D4 角色一致性。** D3 已能在剧本页改结局悬念，未锁镜更新、已锁整镜不动。完整计划见下文 **漫剧智能体制作流水线（D0–D8）**。
...
P0–P8 + D0–D3 已齐。业务继续放插件与 `/api/drama/*`，**不要改 `agent/loop.py`**。

下一刀 **D4**：角色卡 + 参考图进 prompt；每镜选角色；音色绑定。验收：同一角色连续 5 镜比现在稳，可锁参考图。
...
检查器目标：**保存 / 仅重做这一层 / 解锁 / 回滚版本**。D1 已有保存 + 重渲本镜；D2 已有锁与分层重做；D3 已有锁定整镜与剧本影响提示。回滚版本仍待做。
