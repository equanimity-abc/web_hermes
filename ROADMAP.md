...
**漫剧草稿成片（P7–P8 + D0）已齐：** 插件 `tiktok_drama` 能立项 / 写 bible·大纲·分集，解析 `shots.json`，按镜出 clip 再拼 9:16 mp4；`rerender_shot` 只重渲一镜。

**下一刀：D2 分层重做 + 锁。** 聊天改 Markdown 再整集重渲，做不到精细干预；D1 工作台已能改对白/运镜/时长。完整计划见下文 **漫剧智能体制作流水线（D0–D8）**。
...
| **D2** | 分层重做 + 锁 | 约 1 周 | `locked` + 指定层重渲 | 锁画面后改台词只换声/字幕 | 下一刀 |
| **D3–D8** | 剧情 / 角色 / 候选 / 时间线 / 队列 / I2V | 见专章 | 导演式干预到镜头层 | 见下文 D0–D8 | 计划中 |
...
P0–P8 + D0–D1 已齐。业务继续放插件与 `/api/drama/*`，**不要改 `agent/loop.py`**。

下一刀 **D2**：镜头锁 + 指定层重渲（`scene | voice | overlay | clip`）。验收：锁住画面后改台词，只换声和字幕。
...
检查器目标：**保存 / 仅重做这一层 / 解锁 / 回滚版本**。D1 已有保存 + 重渲本镜；锁与分层是 D2。
...
| **D2** | `rerender` 指定 `scene \| voice \| overlay \| clip`；`locked` 生效 | 锁住画面后改台词，只换声和字幕 | 下一刀 |
| **D3** | 剧本编辑器；「这句改了影响 Shot 2/3」；一键只重写脏镜 | 改结局悬念，未锁镜更新，已锁镜不动 | 计划 |
| **D4** | 角色卡 + 参考图进 prompt；每镜选角色；音色绑定 | 同一角色连续 5 镜比现在稳，可锁参考图 | 计划 |
