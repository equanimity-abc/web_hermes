...
**漫剧草稿成片（P7–P8 + D0–D8）已齐：** …时间线页改镜序/切点/转场/音量后导出整集；长渲染进后台队列；对已锁关键帧可试 2–3s I2V 运动，失败回退静图运镜。

**下一刀：** D0–D8 流水线已齐，后续可按需接口型/BGM 或新系列能力。完整计划见下文 **漫剧智能体制作流水线（D0–D8）**。
...
后续可按需接 **口型 / BGM**（D8 已交付 I2V + 静图回退；口型/BGM 仍建议单独里程碑）。
...
**D8 已交付 I2V + 静图回退。** 口型/BGM 仍建议单独排期。D0–D8 已齐。
