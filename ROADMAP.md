...
**漫剧草稿成片（P7–P8 + D0–D2）已齐：** 插件 `tiktok_drama` 能立项 / 写 bible·大纲·分集；`shots.json` 单镜重渲；工作台可改对白/运镜；锁住画面后改台词只换声和字幕。

**下一刀：D3 剧情干预。** D2 已能锁画面后改台词，只换声和字幕。完整计划见下文 **漫剧智能体制作流水线（D0–D8）**。
...
| **D3** | 剧情干预 | 约 1 周 | 剧本编辑器 + 脏镜提示 | 改结局悬念，未锁镜更新 | 下一刀 |
| **D4–D8** | 角色 / 候选 / 时间线 / 队列 / I2V | 见专章 | 导演式干预到镜头层 | 见下文 D0–D8 | 计划中 |
...
P0–P8 + D0–D2 已齐。业务继续放插件与 `/api/drama/*`，**不要改 `agent/loop.py`**。

下一刀 **D3**：剧本编辑器；「这句改了影响 Shot 2/3」；一键只重写脏镜。验收：改结局悬念，未锁镜更新，已锁镜不动。
...
检查器目标：**保存 / 仅重做这一层 / 解锁 / 回滚版本**。D1 已有保存 + 重渲本镜；D2 已有锁与分层重做。回滚版本仍待做。
