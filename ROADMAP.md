...
**结论：** 导演工作台与资产合同已经「专业级」；媒体后端与量产效率仍是「试稿级」。下一步是两条并行的主线：**S 系列把名字里的模型变成真适配器（保证「能发」）**，**C 系列把「能留人」做进验收（冲向「能火」）**。见下文 **成片天花板与专业交付（S0–S8）** 与 **内容级天花板（C0–C4）**。

历史阶段（Hermes 学习 / D / Q / R）保留在本文后半，作决策档案；日常跟 S、C 两条主线与文首验收清单。
...
## 内容级天花板：从「能发」到「能火」（C0–C4）

> **这是 S0–S8 没有覆盖的一层。** S 系列默认「模型画质到位 = 专业级」，但它保证的是「**能发**」（名实相符、能跑、能上架）；决定一部抖音漫剧成败的其实是「**能火**」——前 3 秒能不能留住人、节奏有没有起伏、声音叙事强不强。技术 QC 只能证明「片子没毛病」，证明不了「片子有人看」。本节补这一层。

### C0 · 内容 QC：与技术 QC 并行的「能火」验收

| | 技术 QC（已有） | 内容 QC（新增） |
|--|--|--|
| 回答什么 | 片子能不能发 | 片子能不能留人 |
| 指标 | identity / 口型 / 闪烁 / 响度 | 钩子密度 / 节奏方差 / 对白-静默比 / 悬念间隔 / 情绪曲线 |
| 结论 | 通过 / 待修 | **导演笔记**（这集哪里拖、哪里缺钩子），不是 pass/fail |

落地：`drama_qc.py` 增加「叙事指标」脚本（可无 LLM，先用语料规则 + 时长分布统计）；导出前「可发布」闸 = 技术 QC 全绿 **且** 内容 QC 无「致命拖沓」。

### C1 · 声音先行：先做「广播剧母带」，再反推画面

当前顺序是「先出图 → 配音 → 画面凑时长」。抖音漫剧是碎片化场景，**声音叙事占比极高**。反转为：

```text
对白 TTS → BGM 情绪轨 + SFX 卡点 → 得到「纯声音母带」与逐镜节奏
        → 按声音节奏反推每镜时长 / 切点 / 画面
        → 画面只为「撑住声音的情绪」，而不是让配音去迁就凑出来的镜头
```

好处：剧情节奏由声音决定，改词只动声音母带、画面锁死；「声音对、画面才会对」。

### C2 · 剧情价值路由：按「节奏点」而非「镜头类型」分配预算

现有预算只按 `kind`（establishing 便宜 / action 贵）。引入新的路由维度 `beat`（节奏点）：

| beat | 价值 | 模型 | 场景 |
|--|--|--|--|
| `hook` 钩子 | **最高** | 贵一致性 + 真 I2V | 前 3 秒，完播生死线 |
| `twist` 反转 / `climax` 高潮 | 高 | 贵模型 | 情绪峰值 |
| `bridge` 铺垫 / 过场 | 低 | cheap 档 | 省预算 |

预算从「按 kind 限贵模型数量」，升级为「按剧情价值加权，把钱集中在留人点上」。

### C3 · 非破坏性动画中间表示

当前运动/口型焊死成 mp4，改一个口型音素就要整段重渲。把「运动」存为**可编辑参数**（camera 曲线、姿态点、口型音素对齐），渲染是最后的「烘焙」步骤：

```text
shot.motion  = { curve: pan_right(0.0→1.0, ease_out), keys: [...] }
shot.lip     = { phoneme_align: [("我",0.0),("打",0.4),...], mouth: open/closed/... }
```

「改一个口型」= 改一个对齐点，零重渲。类似 After Effects 表达式层，而非焊死渲染。

### C4 · LLM 审片会：叙事级导演笔记

现有 `drama_director` 是**单镜规则启发式**（景别连续数、关键词）。新增一个**整集叙事级审片**：LLM 读「整集 shots 序列 + 对白 + 时长 + 情绪标注」，产出一份导演笔记：

- 这集哪里拖（连续慢节奏且无钩子）
- 哪个反转来得太早/太晚
- 哪个角色情绪线断了

它**不改镜、不加锁**，只给叙事级意见，人再决定改哪镜。与 C0 的内容 QC 互补：C0 给「信号」，C4 给「为什么」。

### 延后（商业化后置）

- **系列资产库复用（制片厂思维）**：角色 LoRA / 音色 / 风格包 / BGM 情绪库做成公司级资产，跨项目复用，而非 project 级每次重建。
- **数据回流闭环**：对接抖音发布后回流完播率 / 3 秒留存，反哺 C0 钩子与节奏阈值，越做越准。

### C 系列顺序

```text
C0（内容 QC 指标，先立「能火」的度量）
  → C1（声音先行，重排核心节奏的生产顺序）
  → C2（根据 C0/C1 的情绪与节奏信息做剧情价值路由）
C3（非破坏动画）与 C4（审片会）可与 C0–C2 并行
```

C 系列不接任何真模型也能先跑起来（C0 用统计、C4 用现有 LLM、C3 是数据表示重构），是**纯工程 + 纯智能**的地基层，建议与 S1–S3（真模型）并行推进。
...
| **Q7** | 约 3–5 天 | 验收页接入 QC 四项；整集「待修/通过」 | 退回单镜；脚本 `skipped` 不能点通过；响度只重 mix | ✅ |
| **Q8** | 约 3–5 天 | 风格包填路由表；成本汇总（字段 Q0 已有） | 切「古风对话」后新镜走角色模型，定场仍便宜 | ✅ |
...
| **R8** | 验收清单页：汇总 fallback/无声/未锁/未过 identity，一键退回 | 一集能不能过一屏看清 | ✅ |
...
| R8 | 扩 `drama_qc.py` + `DramaStudio.vue` | 验收「一屏清单」+ 一键退回 | `qc_episode_checklist` + `reject_all_qc` |
