...
**漫剧草稿成片（P7–P8 + D0–D7）已齐：** …时间线页改镜序/切点/转场/音量后导出整集；长渲染进后台队列，工作台底部任务条可看进度、取消、重试，聊天不被阻塞。

**下一刀：D8 I2V。** D7 已能后台渲 8 镜时聊天仍可用。完整计划见下文 **漫剧智能体制作流水线（D0–D8）**。
...
| **D8** | I2V | 见专章 | 导演式干预到镜头层 | 见下文 D0–D8 | 计划中 |
...
P0–P8 + D0–D7 已齐。业务继续放插件与 `/api/drama/*`，**不要改 `agent/loop.py`**。

下一刀 **D8**：对已锁关键帧做 2–3 秒 I2V 运动，失败回退静图运镜。
