...
| POST | `/api/drama/projects/{slug}/episodes/{n}/shots/{shot}/rerender` | 只重渲该镜；`layers` 可指定 scene/overlay/voice/clip/assemble |
...
工作台：前端侧栏切到「漫剧」，REST `/api/drama/*` 直接改分镜（对白 / 运镜 / 时长），不走 Agent loop。锁住 `scene` 后改台词只重配音和字幕。
